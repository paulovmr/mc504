...
void drawBoatWithOarToLeft(Boat* boat) {
	...
	if (boat->people[qtd - 1] == MICROSOFT_EMPLOYEE) {
		gotoxy(x + 3 * capacity - 1, y);     printf(BOAT "   _" MICROSOFT_EMPLOYEE_COLOR "O ");
	} else {
		gotoxy(x + 3 * capacity - 1, y);     printf(BOAT "   _" LINUX_HACKER_COLOR "O ");
	}
...
void drawBoatWithOarToCenter(Boat* boat) {
	...
	if (boat->people[qtd - 1] == MICROSOFT_EMPLOYEE) {
		gotoxy(x + 3 * capacity - 1, y);     printf(BODY "  __" MICROSOFT_EMPLOYEE_COLOR "O ");
	} else {
		gotoxy(x + 3 * capacity - 1, y);     printf(BODY "  __" LINUX_HACKER_COLOR "O ");
	}
	gotoxy(x + 3 * capacity - 1, y + 1); printf(BOAT " |  " BODY "| ");
	gotoxy(x + 3 * capacity - 1, y + 2); printf(BOAT " | " BODY "/ \\ " BOAT "/ ");
...
void drawBoatWithOarToRight(Boat* boat) {
	...
	if (boat->people[qtd - 1] == MICROSOFT_EMPLOYEE) {
		gotoxy(x + 3 * capacity - 1, y);     printf(BOAT " ___" MICROSOFT_EMPLOYEE_COLOR "O ");
	} else {
		gotoxy(x + 3 * capacity - 1, y);     printf(BOAT " ___" LINUX_HACKER_COLOR "O ");
	}
	gotoxy(x + 3 * capacity - 1, y + 1); printf(BOAT " \\  " BODY "|");
	gotoxy(x + 3 * capacity - 1, y + 2); printf(BOAT "  \\" BODY "/ \\ " BOAT "/");
...
void drawBoat(Boat* boat, pthread_mutex_t* mutex_sail) {
	...
	for (i = 1; i < qtd; i++) {
		if (boat->people[i - 1] == MICROSOFT_EMPLOYEE) {
			gotoxy(x + 3 * i, y + 1); printf(MICROSOFT_EMPLOYEE_COLOR " O ");
		} else {
			gotoxy(x + 3 * i, y + 1); printf(LINUX_HACKER_COLOR " O ");
		}
